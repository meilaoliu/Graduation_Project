...
为了让Gazebo能找到`substation_dae`模型，你需要把它所在的路径（也就是`<workspace-parent>`）添加到Gazebo的模型路径中。

打开你的`.bashrc`文件并添加以下行：

```bash
echo 'export GAZEBO_MODEL_PATH=$GAZEBO_MODEL_PATH:<workspace-parent>' >> ~/.bashrc
source ~/.bashrc
```

我来帮你执行这个命令。
```bash
echo 'export GAZEBO_MODEL_PATH=$GAZEBO_MODEL_PATH:<workspace-parent>' >> ~/.bashrc && source ~/.bashrc
```
### 2. 在World文件中使用你的模型
