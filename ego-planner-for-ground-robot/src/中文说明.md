...
4. `PathPlanner` 使用 Dijkstra 计算相邻目标之间的最短路径。多目标无显式顺序时采用最短路代价贪心排序；用户明确给出顺序时保留该顺序。
5. 全局轨迹模式下，`SegmentScheduler` 只在 `targets[].stop_required=true` 的目标停留，并只在 `photo_required=true` 时拍照；同一设备若只是作为后续路径的途经点，不会重复停留和重复拍照。
6. `RuntimeEventLog` 记录任务执行过程中的段开始、段完成、失败、低电量、返航充电等事件；后续用户在任务执行中追加指令时，这些反馈会作为上下文注入大模型提示词。
7. 低电量返航不再直接给充电口局部目标，而是先在变电站拓扑图上用 Dijkstra 规划到入口点的全局路径；充满后恢复任务时，也会从入口点重新接入下一段拓扑路径。
8. `BatteryPolicy` 会在执行下一段前计算“当前位置到下一目标 + 下一目标到充电点 + 安全余量”的所需续航。如果当前电量按标定续航换算后不足，则先插入充电段，再继续任务。
9. 限时巡检不再把 `duration_minutes` 预估成多遍重复路径。系统先执行一轮目标序列，并在每段事件中记录已用时间、剩余时间；一轮结束后若仍有时间，再从当前位置动态追加下一轮。
10. `WaypointManager` 是旧版逐航点模式，将规划出的 Waypoint 序列逐点发布到 `/move_base_simple/goal`，并通过 `/odom_adjust` 判断是否到达当前目标。
...
“回到入口点”“回到起点”“返回充电口”应解析为 `task_type=return_home`，目标为 `入口点`，且 `photo_required=false`。
...
* 每段终点速度 / 加速度 = 0（与论文一致）
* 对 `duration_minutes`、`until_time` 这类限时任务，只切首轮目标段；后续是否继续巡检由运行时剩余时间决定，避免一开始机械展开几十个重复段
...
* `battery_full_range_m`：满电估计可行驶距离
* `battery_reserve_m`：返航安全余量
* `time_budget_min_remaining_s`：限时任务剩余时间低于该值时不再追加下一轮
