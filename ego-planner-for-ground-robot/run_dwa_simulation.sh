...
MAP_FILE="$SCRIPT_DIR/src/autonomous_exploration_development_environment/src/vehicle_simulator/world/dense_obstacles_map.pgm"
...
# Launch simulation + DWA + trajectory recorder
roslaunch navigation_baseline sim_env_only.launch &
SIM_PID=$!
sleep 8

roslaunch navigation_baseline move_base_dwa.launch &
MB_PID=$!
sleep 3

rosrun navigation_baseline traj_recorder.py \
    _planner_type:=dwa _output_dir:="$BENCHMARK_DIR" &
REC_PID=$!
...
echo "  python3 random_goal_benchmark.py --tag dwa"
echo "=========================================="
