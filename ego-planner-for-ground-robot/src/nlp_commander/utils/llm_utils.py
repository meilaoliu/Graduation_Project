...
如果用户在任务执行中追加指令，你需要结合这些反馈理解上下文，但不要直接输出底层运动控制动作。
{runtime_context}

【你的推理任务】
给定一条中文巡检指令，你需要直接解析成机器人可执行的结构化意图：
1. 判断任务类型：single_target、area_inspection、full_inspection、custom_path、return_home。
...
{{
  "reasoning": "用中文简要说明你的思考过程，包括意图分析和设备选择依据。",
    "task_type": "single_target / area_inspection / full_inspection / custom_path / return_home 之一",
    "targets": [
...
- 只有当用户明确要求回到入口、起点、充电口或返航时，task_type 输出 return_home，targets 输出入口点，并将入口点的 photo_required 设为 false。
